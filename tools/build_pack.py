#!/usr/bin/env python3
"""Build the German pack with the shared builder in vocab-engine.

Equivalent to:
    PYTHONPATH=engine/tools python3 -m packbuilder build --lang de --repo . [--stage ...]

The pipeline lives in engine/tools/packbuilder (core stages) and
engine/tools/packbuilder/langs/de.py (German rules). This repo keeps only the
...
if __name__ == "__main__":
    sys.exit(main(["build", "--lang", "de", "--repo", str(ROOT)] + sys.argv[1:]))
