#!/bin/sh
# Runs every check that must pass before shipping the German pack/site:
#   1. packbuilder check         - pack-level schema/coverage/article checks
#      (engine/tools/packbuilder/qa/check.py with the German spec)
...
PYTHONPATH="${PACKBUILDER_PATH:-engine/tools}" python3 -m packbuilder check --lang de --repo .
...
TMP="$(mktemp /tmp/german_index_check.XXXXXX.html)"
trap 'rm -f "$TMP"' EXIT
